...
export default function AddTakeScreen() {
  ...
  const { projects, logSheets, addLogSheet, updateLogSheet, moveExistingLogBefore, updateFileNumbers, updateTakeNumbers } = useProjectStore();
    ...
    if (soundDup && cameraDup) {
      if (soundDup.existingEntry?.id === cameraDup.existingEntry?.id) {
        ...
        Alert.alert(
          ...
          [
            { text: 'Cancel', style: 'cancel' },
            { text: 'Insert Before', onPress: () => handleAddTakeWithInsertBefore(existingEntry) }
          ]
        );
        return;
      } else {
...
This would break the logging logic and create inconsistencies in the file numbering system. Please adjust your file numbers to avoid conflicts.`,
    ...
    if (!skipDuplicateCheck) {
      ...
      if (takeDup) {
        ...
        const duplicateEntry = takeDup.duplicateAt;
        Alert.alert(
          'Take Number Exists',
          `Take ${takeData.takeNumber} already exists in Scene ${takeData.sceneNumber}, Shot ${takeData.shotNumber}. Do you want to insert before?`,
          [
            { text: 'Cancel', style: 'cancel' },
            {
              text: 'Use Next Take',
            ...
            { text: 'Insert Before', onPress: () => handleAddTakeWithInsertBefore(duplicateEntry) }
          ]
        );
        return;
      }
