  ...
  const handleSaveTake = () => {
    ...
    if (soundDup && cameraDup) {
      if (soundDup.existingEntry?.id === cameraDup.existingEntry?.id) {
        ...
        Alert.alert(
          'Duplicate Detected',
          `Camera and Sound files are duplicates found in ${location}. Please adjust the file numbers to avoid conflicts.`,
          [{ text: 'OK', style: 'default' }]
        );
        return;
      } else {
...
This would break the logging logic and create inconsistencies in the file numbering system. Please adjust your file numbers to avoid conflicts.`,
    ...
    if (duplicateTake) {
      ...
      Alert.alert(
        'Duplicate Detected',
        `A duplicate take was found at ${loc}. Please adjust the take number to avoid conflicts.`,
        [{ text: 'OK', style: 'default' }]
      );
      return;
    }
